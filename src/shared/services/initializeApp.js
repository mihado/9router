...
export async function initializeApp() {
  try {
    ...
    // [hardened fork] Removed unconditional ensureCloudflared() boot-time download.
    // Upstream fetches+executes the cloudflared binary from GitHub at every startup,
    // regardless of whether a tunnel is enabled (default off). The tunnel feature, if
    // ever enabled, still ensures the binary on-demand inside the tunnel-start path
    // (src/lib/tunnel/cloudflare/cloudflared.js). See docs/SECURITY-REVIEW.md (finding 4).
